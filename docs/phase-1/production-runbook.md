...
准备至少35 GB可用磁盘空间。默认全局文件安全上限为30,000,000,000字节，额外空间用于上传临时文件、容器层和系统日志。
...
| `DROPROOM_LONG_TEXT_FILE_THRESHOLD` | `5000`          | 发送长文字自动转 TXT 的字符阈值          |
| `DROPROOM_MAX_FILES_PER_BATCH`      | `50`            | 单批文件数量上限                         |
| `DROPROOM_MAX_BATCH_BYTES`          | `300000000`     | 单批文件总大小上限                       |
| `DROPROOM_MAX_ROOM_FILE_BYTES`      | `1000000000`    | 单房间文件容量上限                       |
| `DROPROOM_GLOBAL_FILE_BYTES`        | `30000000000`   | 全局文件容量上限，必须小于存储卷可用空间 |
| `DROPROOM_OPENAPI_ENABLED`          | `false`         | 生产默认关闭接口文档                     |
| `DROPROOM_LOG_LEVEL`                | `info`          | 故障排查时临时调整                       |

容量必须满足“单批文件上限 ≤ 单房间文件上限 ≤ 全局文件上限”，长文字发送转文件阈值不得大于单条文字上限，API 会在启动时拒绝不一致的配置。房间快照会把文字、长文字转换、批次和房间容量限制下发给 Web，前端无需同步修改常量。
...
正常情况下，房间销毁和 API 启动会自动清理文件。房主解散房间时，如果存在仍在传输的下载响应，仅对应文件会继续保留最多30分钟并计入全局容量，其他文件立即删除。不要在 API 运行时直接删除存储卷中的单个文件，否则内存状态与磁盘状态会不一致。
